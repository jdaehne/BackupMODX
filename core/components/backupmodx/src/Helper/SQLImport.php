...
class SQLImport
{
     ...
     * @return string|true Returns true or ["success" => true, "log" => $log], if the SQL was imported successfully.
     * @throws Exception
     */
    public static function importSQL(modX $modx, $sqlFile, $logQuery = false)
    {
        ...
        # and filter (remove) those lines, beginning with a sql comment token
        $file = array_filter($file, function ($line) {
            return strpos(ltrim($line), '--') !== 0;
        });
        # and filter (remove) those lines, beginning with a sql notes token
        $file = array_filter($file, function ($line) {
            return strpos(ltrim($line), '/*') !== 0;
        });
